...
static void newObst(const Vector3f newPos);
	...
	FlaccoController(float alpha_, float rho_, float v_max_, VectorXf ks, vector<Vector3f> obstPos_,float _distance = 0.4, float _distance_critic = 0.1){
		alpha = alpha_;
		rho = rho_;
		v_max = v_max_;
		K = ks.asDiagonal();
	...
	*/
    ...
    float cost(const Eigen::Vector3f& pos) const;
    int closest(const Eigen::Vector3f& pos) const;
