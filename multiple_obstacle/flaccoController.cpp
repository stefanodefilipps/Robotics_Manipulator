...
Vector3f FlaccoController::eeDisVec(const VectorXf &Pos, const int numberOfObstacle) const {
	Vector3f temp = Pos - obstPos[numberOfObstacle];
	float r = 0.1;
	temp = temp - temp*(r/temp.norm()); 
...
float FlaccoController::cost(const Vector3f& pos) const {
...
int FlaccoController::closest(const Vector3f& pos) const {
    int numObst{static_cast<int> (obstPos.size())};
    float min{eeDis(pos)};
    float temp{0};
    int closest = 0;
