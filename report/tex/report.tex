...
\item The third is on the second link axis at a \todo distance from the origin of the DH reference frame associated to the 2$^{nd}$ joint (i.e. a bit off the shoulder). In that reference frame, its position is:
...
\section{Results}
We organized the experiment accordingly with the distance of the end-effector from the obstacle and with its obstruction on the path. We simulated 3 different situations with 2 different control algorithms for a total of 6 different simulations.
...
As already said, this particular choice of the obstacle position was meant to create a conflict in the control algorithm, on one side the end effector feels relatively safe with respect to the obstacle since its distance from it is quit large at the beginning, on the other hand the third control point stands right in the \emph{dangerous} region, hence the repulsive velocity it's subject to is quite strong even if it can't receive a higher priority, not being in the \emph{critical} region.
...
Those simulations provide quite interesting results which not always are overall decisive. 
